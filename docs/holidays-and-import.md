...
Status: current as of M1 (2026-09-17). The rule engine (§2.2) exists in `:core:domain` and the packs and loader (§2.4, §2.5) in `:core:holidays`; see [adr/0003-holiday-rule-model.md](adr/0003-holiday-rule-model.md) and [adr/0004-holiday-pack-format.md](adr/0004-holiday-pack-format.md) for the decisions this document left open. The `calendar` rule type (§2.3) is not implemented yet (FEATURES H4).
