...
| `ACCESS_NETWORK_STATE` | Implicit | — | Normal. | Merged in by WorkManager for constraints (*merged-manifest contents not verified; check the build*). Harmless without `INTERNET`. |
...
| `WAKE_LOCK`, `FOREGROUND_SERVICE` | Implicit only | — | Normal; merged by WorkManager. | No foreground services of our own. |
